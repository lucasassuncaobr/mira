...
from server import build_compact_context, build_context, build_prodata_prompt, contextual_fallback, known_prodata_reply, mimo_generate, resolve_model, save_learning
...
def clean_context(text):
    ...
    for raw in (text or "").splitlines():
        ...
        if not lines or lines[-1] != line:
            lines.append(line)
    # Aumentado de 12 para 24 linhas para melhor acompanhamento de conversas longas
    return "\n".join(lines[-24:])
...
class FloatingAgent(Gtk.ApplicationWindow):
    def __init__(self, app):
        ...
        subtitle = Gtk.Label(label="Sugestão automática")
        ...
        close.add_css_class("header-close")
        ...
        response_label = Gtk.Label(label="Sugestão")
        response_label.add_css_class("section-label")
        response_label.set_xalign(0)
        response_section = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        response_section.append(response_label)
        self.response = self.text_view(210, "response-box")
        response_section.append(self.response["scroll"])
        root.append(response_section)
    ...
    def generate_from_context(self):
        ...
        self.set_status("Gerando resposta com MiMo...")
    ...
    def generate_worker(self, context):
        ...
        model = resolve_model()
        prompt = build_prodata_prompt("", "", "profissional", build_compact_context(), context, fast=True)
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(mimo_generate, prompt, model, 12.0)
                result = future.result(timeout=10)
            GLib.idle_add(self.finish_generate, result, "")
        except concurrent.futures.TimeoutError:
            fallback = self.contextual_fallback(context)
    ...
    def finish_generate(self, result, error):
        ...
        if error:
            ...
            if bad_suggestion(result):
                context = text_from(self.context["view"])
                result = self.contextual_fallback(context)
