...
LOCAL_QWEN_TRIAGE_ENABLED = os.environ.get("LOCAL_QWEN_TRIAGE_ENABLED", "0").strip().lower() not in {"0", "false", "no"}
...
LOCAL_QWEN_RESPONSE_ENABLED = os.environ.get("LOCAL_QWEN_RESPONSE_ENABLED", "1").strip().lower() not in {"0", "false", "no"}
...
def greeting_reply(text: str) -> str:
    clean = _normalize_manual_text(text)
    if not clean:
        return ""
    if re.fullmatch(r"(oi|ol[aá]|eai|e ai)([.!?])?", clean):
...
def greeting_prefix(text: str) -> str:
    clean = _normalize_manual_text(text)
    if re.match(r"^bom dia\b", clean):
        return "Bom dia, tudo bem?"
    if re.match(r"^boa tarde\b", clean):
        return "Boa tarde, tudo bem?"
    if re.match(r"^boa noite\b", clean):
...
def _get_greeting_for_response(text: str = "") -> str:
    if "bom dia" in text.lower():
        return "Bom dia"
    if "boa tarde" in text.lower():
        return "Boa tarde"
    if "boa noite" in text.lower():
        return "Boa noite"
...
def _load_moondream_model():
    if not MOONDREAM_ENABLED or md is None:
        return None
    try:
        kwargs = {"local": True}
        if MOONDREAM_DEVICE:
            kwargs["device"] = MOONDREAM_DEVICE
        return md.vl(**kwargs)
    except Exception:
        return None
...
def _load_local_qwen_triage():
    if not LOCAL_QWEN_TRIAGE_ENABLED:
        return None
...
def _load_local_qwen_response():
    if not LOCAL_QWEN_RESPONSE_ENABLED:
        return None
        ...
        [
            {
                "role": "system",
                "content": (
                    "Voce e o Qwen Local do SIG Prodata, usado como triagem e como segunda opiniao do atendente. "
...
def build_local_qwen_prompt(
    ...
    parts = [
        "Voce e o Qwen Local do SIG Prodata.",
...
def extract_last_customer_message(text: str) -> str:
    ...
    for line in reversed(lines):
        cleaned = re.sub(r"^(cliente|eu):\s*", "", line, flags=re.I).strip()
        if cleaned:
            return cleaned
...
ACCESS_PATTERNS = [
    ...
    r"n[aã]o\s+consigo\s+(?:logar|entrar|acessar)",
    r"acesso\s+(?:ao|ao\s+)(?:usuario|usu[aá]rio)",
    r"(?:usuario|usu[aá]rio)\s+(?:bloqueado|travado|cancelado)",
    r"perdi\s+(?:o\s+)?(?:meu\s+)?(?:usuario|usu[aá]rio|acesso|senha)",
    r"bloquearam\s+(?:o\s+)?(?:meu\s+)?(?:usuario|usu[aá]rio)",
    r"minha\s+(?:conta|senha|usuario|usu[aá]rio)",
    r"(?:login|senha|usuario|usu[aá]rio)\s+(?:n[aã]o\s+)?(?:funciona|valido|v[aá]lido|errado|invalido|inv[aá]lido)",
    r"n[aã]o\s+(?:entro|consigo\s+entrar)",
    r"senha\s+(?:errada|invalida|inv[aá]lida)",
...
def build_learning_skills(limit: int = 6) -> str:
    ...
    for raw in reversed(records[-80:]):
        ...
        if q_lower.startswith(("bom dia", "boa tarde", "boa noite", "olá", "ola")):
...
def save_learning(cliente: str, rascunho: str, resposta: str, aprendizado: str) -> None:
    if not is_valid_pair(cliente, resposta):
        return
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    records_dir = ROOT / "dados"
    records_dir.mkdir(exist_ok=True)
    record = {"data": now, "cliente": cliente, "rascunho": rascunho, "resposta_aprovada": resposta, "aprendizado": aprendizado}
...
def generate_prodata_response_details(
    ...
    if lightweight_reply and _is_lightweight_turn(current_query):
    ...
    try:
        ...
        local_qwen_prompt = build_local_qwen_prompt(current_query, conversation_context, local_guidance, visual_context)
        local_qwen_future = executor.submit(local_qwen_generate, local_qwen_prompt, 120) if LOCAL_QWEN_RESPONSE_ENABLED else None
        mimo_future = executor.submit(mimo_generate, prompt, model, mimo_timeout if fast else max(mimo_timeout, 10.0))
        futures = tuple(
            (name, future)
            for name, future in (("qwen", qwen_future), ("qwen_local", local_qwen_future), ("mimo", mimo_future))
            ...
            for name, future in futures:
                ...
                if result:
                    ...
                    if name in {"mimo", "qwen"} and not first_ai_candidate_at:
                        ...
                        if has_visual_context and name == "mimo" and candidate.get("judge_score", 0) >= 4:
                            return _finalize_candidates(candidates, errors, started_at, cliente, rascunho, modo)
                        if name == "qwen" and candidate.get("judge_score", 0) >= 4:
                            return _finalize_candidates(candidates, errors, started_at, cliente, rascunho, modo)
                        if candidate.get("judge_score", 0) >= 5:
                            return _finalize_candidates(candidates, errors, started_at, cliente, rascunho, modo)
                        if candidate.get("judge_score", 0) >= 2 and not local_seed:
                            return _finalize_candidates(candidates, errors, started_at, cliente, rascunho, modo)
            if len(seen_sources) == len(futures):
                break
            now = time.monotonic()
            if first_ai_candidate_at and (now - first_ai_candidate_at) >= ENGINE_SETTLE_SECONDS:
                return _finalize_candidates(candidates, errors, started_at, cliente, rascunho, modo)
            time.sleep(0.03)
        ...
        for name, future in futures:
            if name in seen_sources:
                continue
            wait_limit = 16.0 if name == "mimo" else 3.0
            try:
            ...
            if result:
                elapsed_ms = int((time.monotonic() - started_at) * 1000)
                candidates.append(_build_candidate(name, result, elapsed_ms, current_query, conversation_context, has_visual_context))
    finally:
...
def generate_prodata_preview_details(
    ...
    preview_source = ""
    if not LOCAL_AI_ONLY_TEST:
        preview_text = local_response_reply(cliente, rascunho, modo, conversation_context)
        preview_source = "banco_local" if preview_text else ""
    if not preview_text and current_query:
...
def mimo_generate(prompt: str, model: str | None = None, timeout: float = 20.0) -> str:
    try:
        ...
        local_timeout = max(timeout, 15.0)
        try:
            result = subprocess.run(["mimo", "run", "--model", local_model, prompt], text=True, capture_output=True, env=env, timeout=local_timeout, check=False)
