...
from server import contextual_fallback, generate_prodata_response, save_learning
...
class ProdataWindow(Gtk.ApplicationWindow):
    def __init__(self, app):
        ...
        self.preview_reply_bubble = None
    ...
    def append_message(self, role, text, variant=None, track_history=True):
        if track_history:
            self.message_history.append({"role": role, "text": text or ""})
    ...
    def set_preview_reply(self, text):
        text = (text or "").strip() or "Pensando..."
        if self.preview_reply_bubble is None or self.preview_reply_bubble.get_parent() is None:
            self.preview_reply_bubble = self.append_message("assistant", text, track_history=False)
        else:
            self.preview_reply_bubble.set_text(text)
        return False

    def build_conversation_context(self, limit=12):
        lines = []
        for item in self.message_history[-limit:]:
            text = (item.get("text") or "").strip()
            if not text:
                continue
            label = "CLIENTE" if item.get("role") == "user" else "EU"
    ...
    def on_generate(self, _button):
        ...
        self.set_preview_reply("Pensando...")
        self.set_status("Processando em segundo plano...")
    ...
    def generate_worker(self, cliente, rascunho, modo):
        ...
        try:
            result, _source = generate_prodata_response(
                cliente,
                rascunho,
                modo,
    ...
    def finish_generate(self, result, error):
        ...
        if error:
            ...
            if self.last_reply:
                self.set_preview_reply(self.last_reply)
